...
imperv_2006 |> st_apply(3, mean) |> pull(1) -> x

calc_f = function(ds, cellsize) {
  st_downsample(imperv_2006, c(ds,ds)) |> st_apply(3, mean) |> pull(1) -> y
  e = se[[cellsize]]
...
# Native is 20m
ds = c(1, 3, 4, 9, 19, 39, 49, 99, 199, 399, 499)
cs = c( "40", "80", "100", "200", "400", "800", "1000", "2000", "4000", "8000", "10000")
all = do.call(rbind, mapply(calc_f, ds, cs, SIMPLIFY = FALSE))
```

```{r}
stars_tibble <- as_tibble(imperv_2006, as_points = FALSE)

# Print the structure of the tibble to understand its format
str(stars_tibble)

# Assuming your value column is named 'value' (replace with actual column name)
# Count occurrences of value 1
count_of_ones <- stars_tibble %>%
  summarize(n = sum(V1 == 1, na.rm = TRUE))  # Replace 'V1' with the correct column name for raster values
...
cl <- makeCluster(detectCores() - 1) # Leave one core for other tasks
registerDoParallel(cl)

# Assume raster_data is your stars object and we want to parallelize across multiple bands
results <- foreach(i =1) %dopar% {stars::st_downsample(imperv_2006, c(999,999))}
...
```{r}
year = c("2006","2009","2012","2015")

impervious <- do.call(c, list(imperv_2006,imperv_2009,imperv_2012,imperv_2015))
